package org.splendor.wrapper;

import org.splendor.callback.DefaultCallback;
import org.splendor.callback.ICallback;
import org.splendor.callback.IWorker;
import org.splendor.exception.SkippedException;
import org.splendor.timer.SystemClock;
import org.splendor.worker.DependWrapper;
import org.splendor.worker.ResultState;
...
public class WorkerWrapper<T , V> {
    ...
    private WorkerWrapper(String id, IWorker<T, V> worker, T param, ICallback<T, V> callback) {
        ...
        //允许不设置回调
        if (callback == null) {
            callback = (ICallback<T, V>) new DefaultCallback();
        }
